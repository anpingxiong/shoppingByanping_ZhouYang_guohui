...
public class ShowModifiedBigSortServlet extends HttpServlet {

	public void doGet(HttpServletRequest request, HttpServletResponse response)
			throws ServletException, IOException {
		System.out.println("已进入呈现被修改的大类方法");
		int sid =(Integer.parseInt( request.getParameter("sid")));
		Sort sort = SortServiceImpl.getInstance().receiveSortBySid(sid);
		request.setAttribute("sort", sort);
		request.getRequestDispatcher("./manage/bigSorts_modify.jsp").forward(request, response);
